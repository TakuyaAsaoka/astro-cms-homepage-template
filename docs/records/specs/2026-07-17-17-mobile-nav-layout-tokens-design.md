...
- ヒーローのフルブリード・ハックを除去:
  ...
  - `.home-hero` は全幅背景・アニメーションを保持したまま、内側に `.hero-inner`（`max-width: var(--content-max-width); margin-inline: auto; padding-inline: var(--content-pad); height: 100%;`）を挟み、`.hero-content` を載せて左端を本文と一直線にする
  - **`.hero-inner` の `height: 100%` は必須**。`.hero-content` は `justify-content: flex-end` でヒーロー（70vh）の下端に本文を固定しているため、`.hero-inner` が高さを継承しないと下端揃えが崩れる
- 本文リンク装飾の基準を変更:
...
- マークアップ: `.footer-inner` 内の JSX を**並べ替える**。現状は social-links → copyright の順だが、両端配置で © を左に置くため copyright（先頭）→ social-links の順に変更する（restyle だけでなく順序変更が必要）
- スタイル:
