...
- Modify: `public/admin/config.yml`（コメント内の旧値 `homepage` 例を統一語彙に更新）
...
- [ ] **Step 6: `public/admin/config.yml` のコメント内の旧値 `homepage` 例を統一語彙に更新**

コメント内に旧 `base` 由来の `homepage` が例示として残っており、`public_folder: "/your-repo/images"` と不整合。以下を更新する:
...
7行目（同期コメントの1行目、`例:` の部分のみ更新。他の文言は維持）:
```yaml
# 公開URLのプレフィックス。CMSが本文に挿入する画像パス（例: /homepage/images/x.png）に使う。
```
→
```yaml
# 公開URLのプレフィックス。CMSが本文に挿入する画像パス（例: /your-repo/images/x.png）に使う。
```

- [ ] **Step 7: Prettier で整形しコミット**

Run: `npm run format && npm run format:check`
Expected: format:check が PASS（整形差分なし）。

```bash
git add README.md docs/custom-domain-ssl.md public/admin/config.yml
git commit -m "docs: READMEとドキュメント・CMSコメントの手順を中立化し表記を統一（#19）"
```
